...
{
    public class aAccountController : BaseAPIController
    {
        IUserManager _userManager;
        //IPushNotificationManager _pushNotificationManager;
        IEmailTemplateManager _emailtemplaateManager;
        ...
        [SkipAuthorization, SkipAuthentication]
        public HttpResponseMessage login([FromBody] LoginModel model)
        {
            ...
            var result = _userManager.AuthenticateUser(model);
            if (result != null)
            {
                if (result.Object.IsDeleted == true)
                {
                    return new JsonContent("Your Account has been deleted temporarily", Status.DeletedUserCode, result).ConvertToHttpResponseOK();
                }
                else
                {
                    var token = CreateNewSession(result.Object.UserID);
                    result.Object.SessionId = Guid.Parse(token);
                    return new JsonContent(Messages.LoginSuccess, Status.Success, new { User = result }).ConvertToHttpResponseOK();
                }
            }
            else
        ...
        //    if (result != null)
        //    {
        //        var token = CreateNewSession(result.UserId);
        //        result.SessionId = Guid.Parse(token);
        //        return new JsonContent(Messages.RegisterSuccess, Status.Success, new { user = result }).ConvertToHttpResponseOK();
        //    }
        //    else
        //    {
